...
def insert_data(conn, timestamp, ph, temperature, ec, tds, water_level):
...
def aggregate_and_store(conn):
    """Aggregate sensor data and store the average in the database."""
    global data_buffer

    # Ensure there is data to aggregate
    if not all(data_buffer[key] for key in data_buffer):
        print("Buffer has insufficient data to aggregate.")
        return
